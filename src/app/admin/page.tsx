...
export default function AdminPage() {
  ...
  }, []);  useEffect(() => {    if (!adminMapsReady || !adminMapRef.current || !activeDrive || !activeDrive.driver_lat || !activeDrive.pickup_lat || !activeDrive.dropoff_lat) return;    const m = adminMapRef.current;    async function drawRoute() {      const from = `${activeDrive.driver_lng},${activeDrive.driver_lat}`;      const to = `${activeDrive.dropoff_lng},${activeDrive.dropoff_lat}`;      const via = `${activeDrive.pickup_lng},${activeDrive.pickup_lat}`;      try {        const res = await fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${from};${via};${to}?alternatives=false&geometries=geojson&access_token=${ADMIN_MAPBOX_TOKEN}`);        const data = await res.json();        if (!data.routes || !data.routes[0]) return;        const geom = data.routes[0].geometry;        if (m.getSource('route-source')) { m.removeLayer('route-layer'); m.removeSource('route-source'); }        m.addSource('route-source', { type: 'geojson', data: { type: 'Feature', geometry: geom } });        m.addLayer({ id: 'route-layer', type: 'line', source: 'route-source', paint: { 'line-color': '#0080ff', 'line-width': 5 } }, 'poi-label');      } catch (e) { console.log('Route error:', e); }    }    drawRoute();  }, [activeDrive, adminMapsReady, ADMIN_MAPBOX_TOKEN]);;  useEffect(() => {    if (!adminMapsReady || !adminMapRef.current || !activeDrive || !activeDrive.driver_lat || !activeDrive.pickup_lat || !activeDrive.dropoff_lat) return;    const m = adminMapRef.current;    async function drawRoute() {      const from = `${activeDrive.driver_lng},${activeDrive.driver_lat}`;      const to = `${activeDrive.dropoff_lng},${activeDrive.dropoff_lat}`;      const via = `${activeDrive.pickup_lng},${activeDrive.pickup_lat}`;      try {        const res = await fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${from};${via};${to}?alternatives=false&geometries=geojson&access_token=${ADMIN_MAPBOX_TOKEN}`);        const data = await res.json();        if (!data.routes || !data.routes[0]) return;        const geom = data.routes[0].geometry;        if (m.getSource('route-source')) { m.removeLayer('route-layer'); m.removeSource('route-source'); }        m.addSource('route-source', { type: 'geojson', data: { type: 'Feature', geometry: geom } });        m.addLayer({ id: 'route-layer', type: 'line', source: 'route-source', paint: { 'line-color': '#0080ff', 'line-width': 5 } }, 'poi-label');      } catch (e) { console.log('Route error:', e); }    };  useEffect(() => {    if (!adminMapsReady || !adminMapRef.current || !activeDrive || !activeDrive.driver_lat || !activeDrive.pickup_lat || !activeDrive.dropoff_lat) return;    const m = adminMapRef.current;    async function drawRoute() {      const from = `${activeDrive.driver_lng},${activeDrive.driver_lat}`;      const to = `${activeDrive.dropoff_lng},${activeDrive.dropoff_lat}`;      const via = `${activeDrive.pickup_lng},${activeDrive.pickup_lat}`;      try {        const res = await fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${from};${via};${to}?alternatives=false&geometries=geojson&access_token=${ADMIN_MAPBOX_TOKEN}`);        const data = await res.json();        if (!data.routes || !data.routes[0]) return;        const geom = data.routes[0].geometry;        if (m.getSource('route-source')) { m.removeLayer('route-layer'); m.removeSource('route-source'); }        m.addSource('route-source', { type: 'geojson', data: { type: 'Feature', geometry: geom } });        m.addLayer({ id: 'route-layer', type: 'line', source: 'route-source', paint: { 'line-color': '#0080ff', 'line-width': 5 } }, 'poi-label');;  useEffect(() => {    if (!adminMapsReady || !adminMapRef.current || !activeDrive || !activeDrive.driver_lat || !activeDrive.pickup_lat || !activeDrive.dropoff_lat) return;    const m = adminMapRef.current;    async function drawRoute() {      const from = `${activeDrive.driver_lng},${activeDrive.driver_lat}`;      const to = `${activeDrive.dropoff_lng},${activeDrive.dropoff_lat}`;      const via = `${activeDrive.pickup_lng},${activeDrive.pickup_lat}`;      try {        const res = await fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${from};${via};${to}?alternatives=false&geometries=geojson&access_token=${ADMIN_MAPBOX_TOKEN}`);        const data = await res.json();        if (!data.routes || !data.routes[0]) return;        const geom = data.routes[0].geometry;        if (m.getSource('route-source')) { m.removeLayer('route-layer'); m.removeSource('route-source'); };  useEffect(() => {    if (!adminMapsReady || !adminMapRef.current || !activeDrive || !activeDrive.driver_lat || !activeDrive.pickup_lat || !activeDrive.dropoff_lat) return;    const m = adminMapRef.current;    async function drawRoute() {      const from = `${activeDrive.driver_lng},${activeDrive.driver_lat}`;      const to = `${activeDrive.dropoff_lng},${activeDrive.dropoff_lat}`;      const via = `${activeDrive.pickup_lng},${activeDrive.pickup_lat}`;      try {        const res = await fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${from};${via};${to}?alternatives=false&geometries=geojson&access_token=${ADMIN_MAPBOX_TOKEN}`);        const data = await res.json();        if (!data.routes || !data.routes[0]) return;;  useEffect(() => {    if (!adminMapsReady || !adminMapRef.current || !activeDrive || !activeDrive.driver_lat || !activeDrive.pickup_lat || !activeDrive.dropoff_lat) return;    const m = adminMapRef.current;    async function drawRoute() {      const from = `${activeDrive.driver_lng},${activeDrive.driver_lat}`;      const to = `${activeDrive.dropoff_lng},${activeDrive.dropoff_lat}`;      const via = `${activeDrive.pickup_lng},${activeDrive.pickup_lat}`;      try {        const res = await fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${from};${via};${to}?alternatives=false&geometries=geojson&access_token=${ADMIN_MAPBOX_TOKEN}`);;  useEffect(() => {    if (!adminMapsReady || !adminMapRef.current || !activeDrive || !activeDrive.driver_lat || !activeDrive.pickup_lat || !activeDrive.dropoff_lat) return;    const m = adminMapRef.current;    async function drawRoute() {      const from = `${activeDrive.driver_lng},${activeDrive.driver_lat}`;      const to = `${activeDrive.dropoff_lng},${activeDrive.dropoff_lat}`;      const via = `${activeDrive.pickup_lng},${activeDrive.pickup_lat}`;;  useEffect(() => {    if (!adminMapsReady || !adminMapRef.current || !activeDrive || !activeDrive.driver_lat || !activeDrive.pickup_lat || !activeDrive.dropoff_lat) return;    const m = adminMapRef.current;    async function drawRoute() {      const from = `${activeDrive.driver_lng},${activeDrive.driver_lat}`;;  useEffect(() => {    if (!adminMapsReady || !adminMapRef.current || !activeDrive || !activeDrive.driver_lat || !activeDrive.pickup_lat || !activeDrive.dropoff_lat) return;    const m = adminMapRef.current;;  useEffect(() => {;;
